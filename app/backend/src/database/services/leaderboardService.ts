...
export const leaderboardAway = async () => {
  ...
  const leaderboard = allTeams.map((t) => {
    const matches = allMatches.filter((m) => m.awayTeamId === t.id && m.inProgress === false);
    const winMatches = matches.filter((m) => Number(m.awayTeamGoals) > Number(m.homeTeamGoals));
    const drawMatches = matches.filter((m) => Number(m.homeTeamGoals) === Number(m.awayTeamGoals));
    return {
      name: t.teamName,
      totalPoints: (winMatches.length * 3) + drawMatches.length,
      ...
      totalDraws: drawMatches.length,
      totalLosses: matches.length - winMatches.length - drawMatches.length,
      goalsFavor: matches.reduce((acc, crr) => acc + crr.awayTeamGoals, 0),
      goalsOwn: matches.reduce((acc, crr) => acc + crr.homeTeamGoals, 0),
    };
  });
